...
Apr 5, 2020[Java](/categories/Java)

# 1.JVM的生命周期[](2020/04/05/Java_JVM#1-JVM的生命周期)
...
![](/2020/04/05/Java_JVM/jvm.png)

# 2.类加载器子系统Class Loader

## 2.1 类加载过程[](2020/04/05/Java_JVM#2-1-类加载过程)
...
![](/2020/04/05/Java_JVM/load_process.png)

  * **初始化** ：初始化阶段是执行类构造器方法()的过程，此方法不用定义，是javac编译器自动收集类中的**类变量赋值动作** 和**静态代码块中的语句** 合并而来，不同于类的构造器（JVM中的()），并且父类的()在子类前执行

## 2.2 类加载器[](2020/04/05/Java_JVM#2-2-类加载器)
...
## 2.3 双亲委派机制[](2020/04/05/Java_JVM#2-3-双亲委派机制)
...
![](/2020/04/05/Java_JVM/parent.png)
...
# 3.运行时数据区Runtime Data Area[](2020/04/05/Java_JVM#3-运行时数据区Runtime-Data-Area)
...
## 3.1 程序计数器（PC寄存器）[](2020/04/05/Java_JVM#3-1-程序计数器（PC寄存器）)

JVM中的PC寄存器是对物理的PC寄存器的一种抽象模拟，用于存储指向下一条指令的地址，也即将执行的指令代码，由执行引擎读取下一条指令；

![](/2020/04/05/Java_JVM/PCregister.png)
...
## 3.2 虚拟机栈[](2020/04/05/Java_JVM#3-2-虚拟机栈)
...
> ![](/2020/04/05/Java_JVM/virtual.png)
...
## 3.3 本地方法栈[](2020/04/05/Java_JVM#3-3-本地方法栈)
...
# 4.本地方法接口Native Method Interface[](2020/04/05/Java_JVM#4-本地方法接口Native-Method-
Interface)
